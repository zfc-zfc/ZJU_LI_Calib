  ...
  void undistort(const Eigen::Quaterniond& q_G_to_target,
    ...
    if(scan_raw.height > 1){
        ...
        for (int i = 0; i < scan_raw.size(); i++) {
            VPoint vpoint;
            if (pcl_isnan(scan_raw.points[i].x)) {
                vpoint = NanPoint;
                scan_in_target->points[i] = vpoint;
                continue;
            }
            double point_timestamp = scan_raw.points[i].timestamp;
            Eigen::Quaterniond q_Lk_to_G;
            Eigen::Vector3d p_Lk_in_G;
            if (!traj_manager_->evaluateLidarPose(point_timestamp, q_Lk_to_G, p_Lk_in_G)) {
                continue;
            ...
            Eigen::Quaterniond q_LktoL0 = q_G_to_target * q_Lk_to_G;
            Eigen::Vector3d p_Lk(scan_raw.points[i].x, scan_raw.points[i].y, scan_raw.points[i].z);
            ...
            vpoint.y = point_out(1);
            vpoint.z = point_out(2);
            vpoint.intensity = scan_raw.points[i].intensity;
            scan_in_target->points[i] = vpoint;
        }
    }
  }
