...
#include <utils/pandarXT_common.h>
...
private:
  ...
  PandarConverter::Ptr p_PandarLidarConvert_;
  ...
  void init() {
    switch (lidar_model_) {
      ...
      case LidarModelType::VLP_16_SIMU:
        ...
        case LidarModelType::PandarXT:
            p_PandarLidarConvert_ = PandarConverter::Ptr(
                    new PandarConverter(PandarConverter::ModelType::PandarXT)
                    );
    ...
    for (rosbag::MessageInstance const m : view) {
      ...
      if (lidar_topic == topic) {
        ...
        if (m.getDataType() == std::string("sensor_msgs/PointCloud2")) {
            ...
            } else if (lidar_model_ == LidarModelType::PandarXT) {
                sensor_msgs::PointCloud2::Ptr scan_msg = m.instantiate<sensor_msgs::PointCloud2>();
                timestamp = scan_msg->header.stamp.toSec();
                p_PandarLidarConvert_->unpack_scan(scan_msg, pointcloud);
            }
        }
      ...
      if (imu_topic == topic) {
        ...
        if(lidar_model_ == PandarXT){
            time = imu_msg->header.stamp.toSec() - 0;
        }
