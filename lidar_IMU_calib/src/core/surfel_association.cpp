...
//    cout << "============got here=============" <<endl;
  ...
  if(height > 1 ){
      ...
      for (int i = 0; i < width * height; i++) {
          if (associatedFlag[i] == -1
              || 0 == scan_raw->points[i].timestamp)
              continue;
          spoint.point = Eigen::Vector3d(scan_raw->points[i].x,
                                         scan_raw->points[i].y,
                                         scan_raw->points[i].z);
          spoint.point_in_map = Eigen::Vector3d(scan_inM->points[i].x,
                                                scan_inM->points[i].y,
                                                scan_inM->points[i].z);
          spoint.plane_id = associatedFlag[i];
          spoint.timestamp = scan_raw->points[i].timestamp;
...
//  cout << "min = "<< box_min.transpose() <<endl;
//  cout << "max = "<< box_max.transpose() <<endl;
  ...
  if(scan->height > 1){
      ...
      std::vector<std::vector<int>> ring_masks_temp(32);
      int width[32] = {0};
...
//          cout << "ring = " << ring << ", ring_masks_temp[ring].size = " << ring_masks_temp[ring].size() << endl ;
      ...
      for (int r = 0; r < 32; r++) {
          ring_masks.push_back(ring_masks_temp[r]);
//          cout << "size = " <<ring_masks[r].size() <<endl;
      }
